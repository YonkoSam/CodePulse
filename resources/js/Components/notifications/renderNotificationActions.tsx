...
const renderNotificationActions = (notification: Notification, setNotifications = null, loading, setLoading) => {
    const markAsRead = (id: string) => {
        setLoading({id, type: 'markAsRead'});
        axios.patch(route('notifications.markAsRead'), {id})
            .then(() => {
                if (setNotifications) {
    ...
    const rejectRequest = (id: string, requestId: number) => {
        ...
        axios.post(route('friend.request.reject'), {request_id: requestId})
            .then(() => {
                Toast.fire({
                    icon: 'success',
    ...
    const acceptRequest = (id: string, requestId: number) => {
        ...
        axios.post(route('friend.request.accept'), {request_id: requestId})
            .then(() => {
                Toast.fire({
                    icon: 'success',
            ...
            .catch(error => {
                Toast.fire({
                    icon: 'error',
                    title: error.response?.data?.message || 'Error accepting friend request'
                });
            })
            .finally(() => {
                setLoading({id: null, type: null});
    ...
    const acceptInvite = (id: string, acceptToken: string) => {
        ...
        axios.get(route('teams.accept_invite', acceptToken))
            .then(() => {
                Toast.fire({
                    icon: 'success',
            ...
            .catch(error => {
                Toast.fire({
                    icon: 'error',
                    title: error.response?.data?.message || 'Error accepting invite'
                });
            })
            .finally(() => {
                setLoading({id: null, type: null});
    ...
    const rejectInvite = (id: string, denyToken: string) => {
        ...
        axios.get(route('teams.deny_invite', denyToken))
            .then(() => {
                Toast.fire({
                    icon: 'success',
                    title: 'Invite denied!'
            ...
            .catch(error => {
                Toast.fire({
                    icon: 'error',
                    title: error.response?.data?.message || 'Error denying invite'
                });
            })
            .finally(() => {
                setLoading({id: null, type: null});
        ...
        case 'App\\Notifications\\FriendRequestNotification':
            return (
                <Stack direction='row' spacing={2}>
                    ...
                    >
                        {isLoading && loading.type === 'acceptRequest' ? <CircularProgress size={24}/> : 'Accept'}
                    </Button>
                    <Button
                        variant="contained"
                        color="error"
                    ...
                    >
                        {isLoading && loading.type === 'rejectRequest' ? <CircularProgress size={24}/> : 'Reject'}
                    </Button>
                </Stack>
            );
        case 'App\\Notifications\\TeamInviteNotification':
            return (
                <Stack direction='row' spacing={2}>
                    ...
                    >
                        {isLoading && loading.type === 'acceptInvite' ? <CircularProgress size={24}/> : 'Accept'}
                    </Button>
                    <Button
                        variant="contained"
                        color="error"
                    ...
                    >
                        {isLoading && loading.type === 'rejectInvite' ? <CircularProgress size={24}/> : 'Reject'}
                    </Button>
                </Stack>
            );
        ...
        case 'App\\Notifications\\LikeNotification':
            return (
                <Stack direction='row' alignItems='center'>
                    ...
                    >
                        {isLoading && loading.type === 'markAsRead' ? <CircularProgress size={24}/> :
                    ...
                    <Link href={`${notification.data.url}?markAsRead=${notification.id}`}>
                        <Button
                            ...
                            disabled={isLoading && loading.type === 'markAsRead'}
                        >
                            {isLoading && loading.type === 'markAsRead' ? <CircularProgress size={24}/> : 'Visit'}
                        </Button>
                    </Link>
                </Stack>
            );
...
const handleDelete = async (id: number, shouldRefresh = true) => {
    axios.delete(route('notifications.destroy', {
        'id': id
    })).then(r => {
        Toast.fire({
            icon: "success",
